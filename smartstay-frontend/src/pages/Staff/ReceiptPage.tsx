...
import { 
  ArrowLeft, Download, Printer, Mail, Phone, Calendar,
  Bed, User, DollarSign, Receipt as ReceiptIcon,
  CheckCircle, Home, AlertCircle, XCircle, FileX
} from 'lucide-react';
...
const ReceiptPage: React.FC = () => {
  ...
  const referrer = location.state?.from || 'guests';
  ...
  const handleGoBack = () => {
    if (referrer === 'reservation') navigate('/staff/reservation');
    else if (referrer === 'frontdesk') navigate('/staff/frontDesk');
    else navigate('/staff/manage-guests');
  };

  // Both buttons do the same: open print dialog (user can Print or Save as PDF)
  ...
  return (
    <>
      {/* MAGIC PRINT CSS - ONLY THIS PAGE */}
      <style>
        {`
          @media print {
        ...
        <div className={`flex-1 transition-all duration-300 ${sidebarCollapsed ? "ml-20" : "ml-[230px]"}`}>
          ...
          {!loading && error && (
            <div className="min-h-screen flex items-center justify-center p-6">
              <div className="text-center bg-white rounded-xl shadow-lg p-8 max-w-md">
                ...
                <button onClick={handleGoBack} className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg">
                  Go Back
                </button>
              </div>
            </div>
          ...
          {!loading && !error && booking && (
            <>
              ...
              <header className="bg-white shadow-sm print:hidden border-b">
                <div className="px-6 py-4 flex justify-between items-center flex-wrap gap-4">
                  ...
                  <div className="flex gap-3">
                    <button onClick={handlePrintOrDownload} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-lg shadow-sm">
                      <Printer size={18} /> Print
                    </button>
                    <button onClick={handlePrintOrDownload} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-lg shadow-sm">
                      <Download size={18} /> Download PDF
                    </button>
                  </div>
                </div>
                <p className="text-center text-xs text-gray-500 pb-3 print:hidden">
                  Tip: After clicking Print/Download → Choose "Save as PDF" in the print dialog
                </p>
              </header>

              {/* Alerts - Hidden on print */}
              <div className="print:hidden px-6 pt-6">
                {isCancelled && (
                  <div className="max-w-5xl mx-auto bg-red-50 border-l-4 border-red-500 rounded-r-lg p-5 mb-6">
                    <div className="flex items-start gap-3">
                      <XCircle className="h-6 w-6 text-red-600 mt-0.5" />
                      <div>
                        <p className="font-bold text-red-900 text-lg">Booking Cancelled</p>
                        <p className="text-red-700">This receipt is for reference only.</p>
                      </div>
                    </div>
                  </div>
                )}
                {!isCancelled && hasNoPayments && (
                  <div className="max-w-5xl mx-auto bg-amber-50 border-l-4 border-amber-500 rounded-r-lg p-5 mb-6">
                    <div className="flex items-start gap-3">
                      <AlertCircle className="h-6 w-6 text-amber-600 mt-0.5" />
                      <div>
                        <p className="font-bold text-amber-900 text-lg">No Payments Recorded</p>
                        <p className="text-amber-700">Total amount due: <strong>RM {booking.totalAmount.toFixed(2)}</strong></p>
                      </div>
                    </div>
                  </div>
                )}
              ...
              {/* THE RECEIPT - ONLY THIS PRINTS */}
              <div className="p-6 print:p-0">
                <div className="max-w-5xl mx-auto">
                  <div id="receipt-to-print" ref={receiptRef} className="bg-white rounded-xl shadow-xl overflow-hidden print:rounded-none print:shadow-none">
                    {/* Header */}
                    <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white p-10 text-center">
                      <div className="text-6xl mb-4">Hotel</div>
                      ...
                      <div className="flex items-center justify-center gap-6 text-sm text-purple-100">
                        <div className="flex items-center gap-2"><Mail size={16} /> support@smartstay.com</div>
                        <div className="flex items-center gap-2"><Phone size={16} /> +60 12-345 6789</div>
                      </div>
                    </div>
                        ...
                        <div>
                          ...
                          {isCancelled ? (
                            ...
                            <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-green-100 text-green-700 text-sm font-bold">
                              <CheckCircle size={14} /> {payments[0].status}
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="p-10">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                        <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
                          <h3 className="font-bold text-lg mb-4 flex items-center gap-2 text-gray-900">
                            <User className="h-5 w-5 text-purple-600" /> Bill To
                          </h3>
                          <div className="space-y-2 text-sm">
                            <p className="font-semibold text-gray-900 text-base">{booking.guest.fullName}</p>
                            <div className="flex items-start gap-2 text-gray-600"><Mail size={14} className="mt-0.5" /> <span>{booking.guest.email}</span></div>
                            <div className="flex items-center gap-2 text-gray-600"><Phone size={14} /> <span>{booking.guest.phoneNumber}</span></div>
                            <div className="flex items-center gap-2 text-gray-600"><ReceiptIcon size={14} /> <span>IC: {booking.guest.icNumber}</span></div>
                          </div>
                        </div>
                        <div className="bg-purple-50 rounded-lg p-6 border border-purple-200">
                          <h3 className="font-bold text-lg mb-4 flex items-center gap-2 text-gray-900">
                            <Home className="h-5 w-5 text-purple-600" /> Hotel Information
                          </h3>
                          <div className="space-y-2 text-sm">
                            <p className="font-semibold text-gray-900 text-base">{booking.room.hotelName}</p>
                            <div className="flex items-center gap-2 text-gray-600"><Bed size={14} /> <span>Room {booking.room.roomNumber} - {booking.room.roomType}</span></div>
                            <div className="flex items-center gap-2 text-gray-600"><User size={14} /> <span>{booking.totalGuests} Guest(s)</span></div>
                            <div className="flex items-center gap-2 text-gray-600"><Calendar size={14} /> <span>{booking.numberOfNights} Night(s)</span></div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mb-10">
                        <h3 className="text-xl font-bold mb-4 pb-2 border-b-2 border-gray-200 flex items-center gap-2">
                          <Calendar className="h-5 w-5 text-purple-600" /> Stay Details
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-gray-50 rounded-lg p-6">
                          <div className="text-center">
                            <p className="text-sm text-gray-600 mb-1">Check-In</p>
                            <p className="font-bold text-gray-900">{new Date(booking.checkInDate).toLocaleDateString('en-MY', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                          </div>
                          <div className="text-center">
                            <p className="text-sm text-gray-600 mb-1">Check-Out</p>
                            <p className="font-bold text-gray-900">{new Date(booking.checkOutDate).toLocaleDateString('en-MY', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                          </div>
                          <div className="text-center">
                      ...
                      <div className="mb-10">
                        <h3 className="text-xl font-bold mb-4 pb-2 border-b-2 border-gray-200 flex items-center gap-2">
                          <DollarSign className="h-5 w-5 text-purple-600" /> Payment Summary
                        </h3>
                        {hasNoPayments ? (
                          <div className="bg-amber-50 border border-amber-200 rounded-lg p-8 text-center">
                            ...
                            <p className="text-amber-700 mb-4">No payment transactions have been made.</p>
                            <div className="bg-white rounded-lg p-4 inline-block">
                              <p className="text-sm text-gray-600 mb-1">Total Amount Due</p>
                              <p className="text-3xl font-bold text-amber-600">RM {booking.totalAmount.toFixed(2)}</p>
                            </div>
                          </div>
                        ) : (
                            ...
                            <table className="w-full">
                              ...
                              <tbody className="divide-y divide-gray-200">
                                <tr>
                                  ...
                                  <td className="px-6 py-4 text-right font-semibold text-gray-900">RM {booking.totalAmount.toFixed(2)}</td>
                                </tr>
                                {payments.map((payment, index) => (
                                  <tr key={payment.paymentId}>
                                    <td className="px-6 py-4 text-gray-700">
                                      Payment {index + 1} ({payment.paymentMethod})
                                      <span className="text-xs text-gray-500 ml-2">• {new Date(payment.paymentDate).toLocaleDateString('en-MY')}</span>
                                    </td>
                                    <td className="px-6 py-4 text-right font-semibold text-green-600">- RM {payment.amount.toFixed(2)}</td>
                                  </tr>
                                ))}
                                <tr className="bg-purple-50 border-t-2 border-purple-200">
                                  <td className="px-6 py-4 font-bold text-gray-900 text-lg">Total Paid</td>
                                  <td className="px-6 py-4 text-right text-lg font-bold text-green-600">RM {totalPaid.toFixed(2)}</td>
                                </tr>
                                {remainingBalance > 0 && (
                                  <tr className="bg-amber-50 border-t border-amber-200">
                                    <td className="px-6 py-4 font-bold text-gray-900">Remaining Balance</td>
                                    <td className="px-6 py-4 text-right font-bold text-amber-600">RM {remainingBalance.toFixed(2)}</td>
                                  </tr>
                                )}
                              </tbody>
                            </table>
                      ...
                      {isCancelled ? (
                        <div className="bg-red-50 border-l-4 border-red-500 rounded-r-lg p-6 mb-8">
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <p className="font-bold text-red-900 text-lg">Booking Cancelled</p>
                              <p className="text-red-700">Refund will be processed according to policy.</p>
                            </div>
                          </div>
                        </div>
                      ) : hasNoPayments ? (
                        <div className="bg-amber-50 border-l-4 border-amber-500 rounded-r-lg p-6 mb-8">
                          <div className="flex items-start gap-3">
                            <AlertCircle className="h-6 w-6 text-amber-600 mt-0.5" />
                            <div>
                              <p className="font-bold text-amber-900 text-lg">Payment Required</p>
                              <p className="text-amber-700">Full payment needed to confirm booking.</p>
                            </div>
                          </div>
                        </div>
                      ) : remainingBalance === 0 ? (
                        <div className="bg-green-50 border-l-4 border-green-500 rounded-r-lg p-6 mb-8">
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <p className="font-bold text-green-900 text-lg">Payment Complete!</p>
                              <p className="text-green-700">Your booking is fully paid and confirmed.</p>
                            </div>
                          </div>
                        </div>
                      ) : (
                        <div className="bg-blue-50 border-l-4 border-blue-500 rounded-r-lg p-6 mb-8">
                          <div className="flex items-start gap-3">
                            <ReceiptIcon className="h-6 w-6 text-blue-600 mt-0.5" />
                            <div>
                              <p className="font-bold text-blue-900 text-lg">Thank you for your payment!</p>
                              <p className="text-blue-700">Remaining: RM {remainingBalance.toFixed(2)} due on check-in.</p>
                            </div>
                          </div>
                        </div>
                      )}

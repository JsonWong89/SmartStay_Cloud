// src/pages/WalkInBookingPage.tsx
import React, { useState, useEffect } from "react";
import {
  UserPlus, Bed, Calendar, Users, CreditCard,
  CheckCircle, Mail, Phone, MapPin, User, DollarSign,
  AlertCircle, FileText, Save, Search, Loader2
} from "lucide-react";
import Sidebar from "../../components/Sidebar";
import { guestsAPI, roomsAPI, bookingsAPI, paymentsAPI } from "../../services/api";
...
function WalkInBookingContent({
  ...
  const [guestInfo, setGuestInfo] = useState<GuestInfo>({
    FullName: "", ICNumber: "", Email: "", PhoneNumber: "", Address: ""
  });

  const [bookingInfo, setBookingInfo] = useState<BookingInfo>({
    RoomID: 0, RoomNumber: "", RoomType: "", PricePerNight: 0,
  ...
  const handleRoomSelect = (room: AvailableRoom) => {
    ...
    if (bookingInfo.NumberOfNights > 0) {
      ...
      setPaymentInfo(p => ({ ...p, DepositAmount: Number((total * 0.3).toFixed(2)) }));
    }
  };
    ...
    setBookingInfo(prev => {
      ...
      if (updated.CheckInDate && updated.CheckOutDate) {
        const nights = Math.ceil(
          (new Date(updated.CheckOutDate).getTime() - new Date(updated.CheckInDate).getTime()) /
          (1000 * 60 * 60 * 24)
        );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!guestRes.success) throw new Error(guestRes.message || "Failed to create guest");
      ...
      if (!bookingRes.success) throw new Error(bookingRes.message || "Failed to create booking");
      const bookingId = bookingRes.data.bookingId;

      // 3. Handle Payment
      if (paymentInfo.PaymentMethod === "Cash") {
        await paymentsAPI.processPayment(bookingId, paymentInfo.DepositAmount, "Cash");
        ...
        await paymentsAPI.confirmStripePayment(bookingId, paymentInfo.DepositAmount);
        alert("Booking & Card Payment Successful!");
      }

      // Reset form
      setGuestInfo({ FullName: "", ICNumber: "", Email: "", PhoneNumber: "", Address: "" });
      setBookingInfo({ RoomID: 0, RoomNumber: "", RoomType: "", PricePerNight: 0, CheckInDate: "", CheckOutDate: "", TotalGuests: 1, NumberOfNights: 0 });
                ...
                <div className="mb-5 p-4 bg-gray-50 rounded-lg space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <button
                      ...
                      className="px-1 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm font-medium transition"
                    >
                      Clear Filters
                    </button>
                  </div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={(e) => setPaymentInfo(prev => ({ ...prev, DepositAmount: parseFloat(e.target.value) || 0 }))}
                    ...
                    <div className="mb-4">
                      ...
                      {paymentInfo.PaymentMethod === "Card" && (
                        <div className="mt-4 p-4 border-2 border-amber-200 rounded-lg bg-amber-50">
                          ...
                          <p className="text-xs text-gray-500 mt-2">Your card is secure and never stored.</p>
                        </div>
                      )}
                    </div>
